...
import { ModularArchBody } from '~/app/types';
...
export const isModArchResponse = <T>(response: unknown): response is ModularArchBody<T> => {
...
export const assembleModularArchBody = <T>(data: T): ModularArchBody<T> => ({
  data,
});
